...
export class DiffManager {
   ...
   */
  async openDiff(filePath: string, force = false): Promise<void> {
    const absPath = normalizePath(filePath);
    ...
    const title = `AI Diff: ${path.basename(absPath)}`;

    // Vẫn cần gọi renderer để tính toán danh sách hunks (giúp render CodeLens)
    this.renderer.show(absPath, snapshot.content, modifiedContent);

    // Respect the `autoOpenDiffEditor` setting for automatic invocations.
    // Manual entry points (Session panel "Open diff", navigation Alt+H/L, command palette)
    // pass force=true to always show the split-view diff.
  ...
  async accept(filePath: string): Promise<void> {
    ...
    if (nextTarget) {
      // User is actively reviewing — always advance into the next pending diff.
      await this.openDiff(nextTarget, true);
    }
